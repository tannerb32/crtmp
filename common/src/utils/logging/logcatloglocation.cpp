...
void LogCatLogLocation::Log(int32_t level, string time, string fileName, uint32_t lineNumber,
	...
	__android_log_write(_levelsMap[level], "rtmpd",
			STR(format("%s:%s:%u %s", STR(time), STR(fileName), lineNumber, STR(message))));
}
