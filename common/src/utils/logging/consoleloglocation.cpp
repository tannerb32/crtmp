...
void ConsoleLogLocation::Log(int32_t level, string time, string fileName,
                             uint32_t lineNumber, string functionName, string message)
{
    ...
    if (_allowColors)
    {
        ...
        cout << time << ":" << fileName << ":" << lineNumber << " " << message;
        SET_CONSOLE_TEXT_COLOR(_colors[6]);
        cout << endl;
    }
    else
    {
        cout << time << ":" << fileName << ":" << lineNumber << " " << message << endl;
    }

#endif /* ANDROID */
}

void ConsoleLogLocation::Log(int32_t level, string time, string fileName, uint32_t lineNumber,
