...
void Logger::Log(int32_t level, string time, string fileName, uint32_t lineNumber,
                 string functionName, string formatString, ...)
{
    ...
    {
        if (_pLogger->_logLocations[i]->EvalLogLevel(level, time, fileName, lineNumber,
                                                     functionName, formatString))
            _pLogger->_logLocations[i]->Log(level, time, fileName,
                                            lineNumber, functionName, message);
    }
}

void Logger::LogProd(int32_t level, string time, string fileName, uint32_t lineNumber, string functionName, Variant &le)
{
    LOCK;
