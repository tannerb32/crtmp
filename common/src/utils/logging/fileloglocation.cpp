...
bool FileLogLocation::EvalLogLevel(int32_t level, string time, string &fileName, uint32_t lineNumber,
                                   string &functionName, string &message)
{
    if (!_canLog)
        return false;

    return BaseLogLocation::EvalLogLevel(level, time, fileName, lineNumber, functionName, message);
}
                                   ...
                                   string functionName, Variant &le)
{
    ...
    return BaseLogLocation::EvalLogLevel(level, time, fileName, lineNumber, functionName, le);
}

void FileLogLocation::Log(int32_t level, string time, string fileName, uint32_t lineNumber,
                          string functionName, string message)
{
    if (_fileIsClosed)
    {
    ...
    string logEntry = format("%"PRIu64":%s:%s:%u:%s:%s",
                             level, STR(time), STR(fileName), lineNumber, STR(functionName),
                             STR(message));
...
void FileLogLocation::Log(int32_t level, string time, string fileName, uint32_t lineNumber, string functionName, Variant &le)
{
    return;
}
